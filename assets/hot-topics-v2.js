(() => {
  ...
  function dynamicFromTitle(card) {
    const raw = (card.querySelector('h3')?.textContent || '')
      .replace(/[“”"':,;!?()]/g, ' ')
      .replace(/\s+/g, ' ')
      .trim();
    if (!raw) return '';
    const useful = raw.split(' ').filter(word => {
  ...
  function buildTopics() {
    ...
    for (const card of recentCards().slice(0,24)) {
      ...
      for (const topic of known) {
        if (text.includes(normalize(topic))) addScore(scores, topic, weight + 60);
      }
      const dynamic = dynamicFromTitle(card);
      if (dynamic) addScore(scores, dynamic, weight);
    }
  ...
  function applyTopic(topic, doScroll = true) {
    ...
    if (empty) {
      ...
      if (!count) empty.textContent = 'No encontramos noticias para este tema.';
    }
  ...
  function ensureStyle() {
    ...
    style.textContent = `
      #rs-hot-topics-v2{width:100%;background:#0c1117;border-bottom:1px solid #232b34;overflow:hidden;position:relative;z-index:20}
      #rs-hot-topics-v2 .rs-hot-inner{max-width:1180px;margin:0 auto;padding:8px 14px;display:flex;align-items:center;gap:8px;overflow-x:auto;scrollbar-width:none;-webkit-overflow-scrolling:touch}
      #rs-hot-topics-v2 .rs-hot-inner::-webkit-scrollbar{display:none}
      #rs-hot-topics-v2 .rs-hot-label{flex:0 0 auto;font-size:11px;font-weight:900;letter-spacing:.08em;color:#ff5a5f}
      #rs-hot-topics-v2 button{flex:0 0 auto;border:0;background:transparent;color:#f3f5f7;font:inherit;font-size:13px;font-weight:750;cursor:pointer;padding:6px 9px;border-radius:999px;white-space:nowrap;touch-action:manipulation;-webkit-tap-highlight-color:transparent}
      #rs-hot-topics-v2 button:hover,#rs-hot-topics-v2 button:focus-visible,#rs-hot-topics-v2 button.active{background:#1a222b;color:#fff;outline:none}
      #rs-hot-topics-v2 .rs-hot-all{color:#aeb7c2}
      @media(max-width:699px){#rs-hot-topics-v2 .rs-hot-inner{padding:7px 10px}#rs-hot-topics-v2 button{font-size:12px;padding:6px 8px}}
    `;
  ...
  function render() {
    ...
    all.className = 'rs-hot-all';
    all.textContent = 'Ver todo';
    inner.append(all);
    bar.replaceChildren(inner);
    setActiveButton();
    if (activeTopic) requestAnimationFrame(() => applyTopic(activeTopic, false));
  }
  ...
  document.addEventListener('ranasports:trends', () => {
    scheduleRender();
    if (activeTopic) setTimeout(() => applyTopic(activeTopic, false), 0);
  });
  document.querySelector('.category-tabs')?.addEventListener('click', () => { activeTopic = ''; });
  search?.addEventListener('input', () => { if (document.activeElement === search) activeTopic = ''; });
