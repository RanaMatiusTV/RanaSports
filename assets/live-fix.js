...
let timer=null,loading=false,mode='all',items=[];
const cache=new Map(),open=new Set(),collapsed=new Set();
...
const apiDay=()=>today().replaceAll('-','');
const clock=d=>new Intl.DateTimeFormat('es-AR',{timeZone:TZ,hour:'2-digit',minute:'2-digit',hour12:false}).format(d);
const label=()=>new Intl.DateTimeFormat('es-AR',{timeZone:TZ,weekday:'short',day:'2-digit',month:'short'}).format(new Date()).replaceAll('.','').toUpperCase();
...
function css(){
 ...
 .rs-board{color:#eef2f6}.rs-top{display:flex;justify-content:space-between;align-items:center;padding:12px 14px;margin-bottom:12px;background:#0c1117;border:1px solid #252d36;border-radius:10px}
 .rs-top b{display:block;color:#ff1746;font-size:10px;letter-spacing:.1em}.rs-top h2{margin:3px 0 0;color:#fff;font-size:1.35rem}.rs-date{font-size:11px;font-weight:900;color:#aeb7c2}
 ...
 @media(max-width:390px){.rs-game{grid-template-columns:50px minmax(0,1fr) 18px}.rs-team{font-size:11px}.rs-score{min-width:40px;font-size:17px}}
 html[data-theme="light"] .rs-top,html[data-theme="light"] .rs-state,html[data-theme="light"] .rs-tab,html[data-theme="light"] .rs-game{background:#fff!important;color:#171b21!important}html[data-theme="light"] .rs-game.live{background:#edf5d3!important}`;document.head.append(s)
}

function shell(m){
 m.dataset.sourceStatus='rana-board-v33';
 m.innerHTML=`<div class="rs-board"><div class="rs-top"><div><b>● PARTIDOS DE HOY</b><h2>RESULTADOS Y PARTIDOS</h2></div><div class="rs-date">${esc(label())}</div></div><div class="rs-tabs"><button class="rs-tab active" data-mode="all">TODOS</button><button class="rs-tab" data-mode="live">EN VIVO</button></div><div id="rs-summary" class="rs-summary"></div><div id="rs-state" class="rs-state">Cargando partidos…</div><div id="rs-list" class="rs-list"></div><div class="rs-foot"><span>Hora Argentina · actualización cada 60 s.</span><a href="https://sportscore.com/" target="_blank" rel="dofollow">Datos en vivo: SportScore ↗</a></div></div>`;
...
const wanted=x=>!!mapComp(x.comp),isToday=x=>{const d=new Date(x.time);return Number.isFinite(d.getTime())&&day(d)===today()},ko=x=>{const d=new Date(x.time);return Number.isFinite(d.getTime())?d.getTime():9e15};
...
function merge(e,s){const r=[...e];for(const x of s){if(!isToday(x)||!wanted(x))continue;const h=r.find(y=>same(y.home,x.home)&&same(y.away,x.away));if(h){h.slug=x.slug||h.slug;h.compLogo=x.compLogo||h.compLogo;h.homeLogo=x.homeLogo||h.homeLogo;h.awayLogo=x.awayLogo||h.awayLogo;if(kind(x)==='live'||kind(x)==='finished'){h.status=x.status;h.statusText=x.statusText;h.homeScore=x.homeScore;h.awayScore=x.awayScore;if(x.minute!==null&&x.minute!==undefined&&x.minute!=='')h.minute=x.minute}}else{x.comp=mapComp(x.comp)||x.comp;r.push(x)}}return r.filter(x=>x.home&&x.away&&isToday(x)).sort((a,b)=>ko(a)-ko(b))}
...
async function fetchEspn(){const settled=await Promise.allSettled(ESPN_LEAGUES.map(fetchEspnLeague));return settled.flatMap(r=>r.status==='fulfilled'?r.value:[])}
...
async function refresh(){if(loading||location.hash!=='#en-vivo')return;const m=document.querySelector('#en-vivo');if(!m)return;if(m.dataset.sourceStatus!=='rana-board-v33')shell(m);loading=true;try{const[e,s]=await Promise.all([fetchEspn(),fetchSport()]);items=merge(e,s);paint(m)}finally{loading=false}}
function install(){if(location.hash!=='#en-vivo')return;const m=document.querySelector('#en-vivo');if(!m)return;css();if(m.dataset.sourceStatus!=='rana-board-v33')shell(m);refresh();clearInterval(timer);timer=setInterval(refresh,60000)}
