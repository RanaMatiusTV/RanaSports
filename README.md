...
## Rediseño editorial
Navegación: INICIO · EN VIVO · FÚTBOL · ROJO · F1 · MÁS DEPORTES · AGENDA. Se conservan los hashes de categorías para no romper enlaces. Rojo filtra Independiente y Selección sigue accesible entre los filtros. La portada destaca la noticia más reciente, tres secundarias y el resto en Más noticias, siempre respetando los filtros y la búsqueda.

assets/redesign.css aplica el diseño mobile-first sobre la base existente. El logo SVG de rana con ojos de pelota también se usa en los iconos PWA. Los módulos breakingFeed, en-vivo y posiciones declaran data-source-status="not-connected": son estructuras visuales sin datos ficticios, sin conexión automática a X y sin API de resultados contratada. El botón Ver en X abre el perfil real. La agenda mantiene su enlace original. La integración futura debe proporcionar una fuente verificada antes de mostrar publicaciones, resultados o posiciones.

## Destacadas por interés de búsqueda
Se consulta el RSS público de Google Trends Argentina mediante .github/workflows/trends.yml cada 30 minutos (GitHub puede demorar ejecuciones). scripts/update-trends.cjs guarda solo consultas, volumen aproximado y fechas en assets/trends.json, sin reproducir artículos externos. La señal identifica tendencias crecientes; no mide todas las búsquedas de Internet.

Destacadas selecciona hasta cuatro noticias publicadas en las últimas 24 horas cuyos títulos/resúmenes contienen los términos significativos de una tendencia también reciente. Se ordenan por volumen aproximado de esa tendencia y luego fecha. El resto se ordena cronológicamente en Últimas Noticias, sin repetir URLs. Datos vencidos, falta de coincidencias o fallo de fuente no se reemplazan por popularidad inventada: todas las noticias pasan a Últimas. No se agregan columnas a Sheets. Las URLs no cambian.
