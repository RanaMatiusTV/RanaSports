const PREFIX = 'ranasports-' + self.registration.scope;
const CACHE = PREFIX + 'v26';
const BUILD = '20260915-2221';
const CORE = ['./', './index.html', './noticia.html', './assets/styles.css', './assets/redesign.css', './assets/portal-list.css', './assets/app.js', './assets/live-fix.js', './assets/navigation.js', './assets/highlights.js', './assets/breaking.js', './assets/hot-topics-v2.js', './assets/live-scores.js', './assets/trends.json', './assets/news-feed.js', './assets/icon.svg', './assets/icon-192.png', './assets/icon-512.png', './assets/apple-touch-icon.png', './manifest.webmanifest', './noticias/bienvenidos.html', './noticias/agenda-deportiva.html', './noticias/rana-f1.html', './legal/acerca-de.html', './legal/contacto.html', './legal/privacidad.html'];
const coreURLs = new Set(CORE.map(path=>new URL(path,self.registration.scope).href));
const APP_URL = new URL('./assets/app.js',self.registration.scope).href;
const LIVE_FIX_URL = new URL('./assets/live-fix.js',self.registration.scope).href;
const NEWS_CSV_HOST='docs.google.com';
const NEWS_CSV_PATH='/spreadsheets/d/e/2PACX-1vRmbZPf_uxPdpS-phGua9U3PccA2z7Uls3G8r49CLfi37qkMJkpRPDUU7VdAZg_IMI7Ynegy-yxyAhr/pub';

async function freshResponse(url){
 const target=new URL(url);
 target.searchParams.set('_rsbuild',BUILD);
 return fetch(target.href,{cache:'reload',credentials:'same-origin'});
}

async function combinedApp(cache){
 try{
  const [appResponse,fixResponse]=await Promise.all([freshResponse(APP_URL),freshResponse(LIVE_FIX_URL)]);
  if(!appResponse.ok)throw new Error('app '+appResponse.status);
  const appText=await appResponse.text();
  const fixText=fixResponse.ok?await fixResponse.text():'';
  const headers=new Headers(appResponse.headers);
  headers.set('content-type','application/javascript; charset=utf-8');
  headers.delete('content-length');
  headers.delete('content-encoding');
  const merged=new Response(appText+'\n\n/* RanaSports EN VIVO v26 */\n'+fixText,{status:200,statusText:'OK',headers});
  await cache.put(APP_URL,merged.clone());
  return merged;
 }catch(error){
  const cachedApp=await cache.match(APP_URL);
  const cachedFix=await cache.match(LIVE_FIX_URL);
  if(cachedApp){
   const appText=await cachedApp.text();
   const fixText=cachedFix?await cachedFix.text():'';
   return new Response(appText+'\n'+fixText,{headers:{'content-type':'application/javascript; charset=utf-8'}});
  }
  return Response.error();
 }
}

self.addEventListener('install',event=>event.waitUntil((async()=>{
 const cache=await caches.open(CACHE);
 await Promise.all(CORE.map(async path=>{
  const canonical=new URL(path,self.registration.scope).href;
  try{const response=await freshResponse(canonical);if(response.ok)await cache.put(canonical,response.clone());}catch{}
 }));
 await self.skipWaiting();
})()));

self.addEventListener('activate',event=>event.waitUntil((async()=>{
 const keys=await caches.keys();
 await Promise.all(keys.filter(key=>key.startsWith(PREFIX)&&key!==CACHE).map(key=>caches.delete(key)));
 await self.clients.claim();
 const clients=await self.clients.matchAll({type:'window',includeUncontrolled:true});
 for(const client of clients){try{await client.navigate(client.url);}catch{}}
})()));

self.addEventListener('fetch',event=>{
 const url=new URL(event.request.url);
 if(event.request.method!=='GET')return;
 if(url.hostname===NEWS_CSV_HOST&&url.pathname===NEWS_CSV_PATH&&url.searchParams.get('output')==='csv'){
  const fresh=new URL(url.href);fresh.searchParams.set('_rs',Date.now().toString());
  event.respondWith(fetch(fresh.href,{cache:'no-store',credentials:'omit'}));
  return;
 }
 if(url.origin!==self.location.origin||!url.href.startsWith(self.registration.scope))return;
 const normalized=new URL(url.href);normalized.search='';
 if(!coreURLs.has(normalized.href))return;
 event.respondWith((async()=>{
  const cache=await caches.open(CACHE);
  if(normalized.href===APP_URL)return combinedApp(cache);
  try{
   const response=await freshResponse(normalized.href);
   if(response.ok&&response.type==='basic')event.waitUntil(cache.put(normalized.href,response.clone()));
   return response;
  }catch{
   return (await cache.match(normalized.href))||Response.error();
  }
 })());
});